...
class CustomPedestrianControl(BasicControl):
    ...
    def __init__(self, actor, args=None):
        ...
        super(CustomPedestrianControl, self).__init__(actor)
        self._targetPoint = 0
        self._lastSpeed = 0
    ...
    def run_step(self):
        ...
        if not self._actor or not self._actor.is_alive:
            return
        try:
            self._target_speed = self._lastSpeed + self._speeds[str(self._targetPoint)]
        except:
            pass
        ...
        if self._waypoints:
            ...
            if direction_norm < 1.0:
                self._lastSpeed = self._target_speed
                self._waypoints = self._waypoints[1:]
                self._targetPoint = self._targetPoint + 1
